---
title: Inter- and intra-animal variation in the integrative properties of stellate
...
- givenNames: Ioannis
  ...
  - name: The Alan Turing Institute
    address:
      ...
      type: PostalAddress
    type: Organization
  - name: School of Mathematics, Maxwell Institute and Centre for Statistics, University
      of Edinburgh
    address:
      addressCountry: United Kingdom
      addressLocality: Edinburgh
...
- givenNames:
  ...
  - name: Centre for Discovery Brain Sciences, University of Edinburgh
    ...
    type: Organization
description:
- content: Distinctions between cell types underpin organizational principles for
    nervous system function. Functional variation also exists between neurons of the
    same type. This is exemplified by correspondence between grid cell spatial scales
    ...
    structured either within or between individuals. Using ex-vivo patch-clamp recordings
    from up to 55 SCs per mouse, we found that integrative properties vary between
    mice and, in contrast to the modularity of grid cell spatial scales, have a continuous
    dorsoventral organization. Our results constrain mechanisms for modular grid firing
    and provide evidence for inter-animal phenotypic variability among neurons of
    the same type. We suggest that neuron type properties are tuned to circuit-level
    set points that vary within and between animals.
  type: Paragraph
isPartOf:
  volumeNumber: '9'
  isPartOf:
...
# The package broomExtra must also be installed.
library(tidyverse)
library(cowplot)
library(lme4)
library(GGally)
...
```{r Figure 1A, fig.cap="(ref:figure1A)"}
...
```{r Figure 4C, warning=FALSE, message=FALSE}
...
```{r Figure 4D, warning=FALSE, message=FALSE}
...
```{r Figure 4E, message=FALSE, warning=FALSE}
